...
/**
 * TRINETHRA DEFENTECH — SOLUTIONS (COMPLETE REDESIGN)
 * ---------------------------------------------------------------------------
 * Design goals
 * 1) Strict color policy: ONLY HSL variables provided by the user (forest, cream, navy, steel, charcoal, signal, alert, teal) + system tokens
 * 2) Device-perfect responsiveness: dvh/svh fallbacks, safe-area in mobile sheets, clamped type, min-w-0 everywhere, no horizontal scroll
 * 3) Modern IA: Overview → Features → Applications → Products → Compare + Sticky TOC (desktop) and condensed TOC (mobile)
 * 4) A11y: real buttons/links, aria-pressed, aria-current, aria-live, keyboard focus rings via shadcn/ui
 * 5) SEO: JSON-LD per list + detail
 * 6) Maintain the SAME data model as your existing code so this is drop-in
 *
 * Tailwind notes: This assumes your Tailwind theme already maps classnames like
 * bg-forest, text-cream, text-navy, text-charcoal, text-steel, etc. to the HSL tokens.
 * If not, replace with bracketed HSL utilities e.g. text-[hsl(var(--navy))].
 */
...
/* ---------------- Data (use your existing objects as-is) ---------------- */
...
/* ---------------- Small UI primitives (tokens only) ---------------- */
const Badge = ({ children, variant = 'secondary', className = '' }: { children: React.ReactNode; variant?: 'default' | 'secondary' | 'destructive' | 'outline'; className?: string }) => (
    <UIBadge
        ...
        variant === 'default' ? 'bg-forest text-cream' : variant === 'secondary' ? 'bg-[hsl(var(--forest)/0.10)] text-forest' : ''
        } ${className}`}
    >
        {children}
    </UIBadge>
...
const Pill = ({ children, active = false, onClick }: { children: React.ReactNode; active?: boolean; onClick?: () => void }) => (
    <button
        type="button"
        onClick={onClick}
        ...
        active
            ...
            : 'bg-[hsl(var(--card))] text-[hsl(var(--steel))] border-[hsl(var(--border))] hover:bg-[hsl(var(--cream))]'
        }`}
        aria-pressed={active}
    >
        {children}
...
const Section = ({ id, title, children }: { id: string; title?: string; children: React.ReactNode }) => (
    <section id={id} className="scroll-mt-[88px] lg:scroll-mt-[112px] min-w-0">
        {title && (
        <h2 className="font-condensed font-bold mb-4 text-navy leading-snug [text-wrap:balance] text-[clamp(1.1rem,3.5vw,1.6rem)]">{title}</h2>
        )}
        {children}
    </section>
);

/* ---------------- Product Tabs + Compare (tokens only) ---------------- */
...
useEffect(() => {
    ...
    const update = (e: MediaQueryList | MediaQueryListEvent) => setIsMobile('matches' in e ? e.matches : (e as MediaQueryList).matches);
    update(mq);
...
const ProductPanel = ({ prod, isComparison = false }: { prod: Product; isComparison?: boolean }) => (
    <Card className="h-full bg-[hsl(var(--card))] border-[hsl(var(--border))]">
        <CardHeader className="pb-4">
            <div className="flex justify-center items-center bg-[hsl(var(--cream))] rounded-lg p-6 mb-4">
            <div className="relative w-full max-w-sm">
                <div className="aspect-[4/3] rounded-lg grid place-items-center">
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-[hsl(var(--forest)/0.08)] grid place-items-center">
                    <svg className="w-8 h-8 sm:w-10 sm:h-10 text-forest" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
                ...
                <div key={k} className="grid grid-cols-3 gap-2 text-sm">
                    ...
                    <div className="text-[hsl(var(--charcoal)/0.8)] whitespace-pre-line col-span-2">{v}</div>
                </div>
                ))}
            </div>
            </div>
      ...
      {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <Button
                ...
                className={`${compareMode ? 'bg-forest text-cream hover:bg-forest/90' : 'text-navy border-[hsl(var(--border))]'} `}
            ...
            {canCompare && (
                ...
                className="bg-teal hover:bg-[hsl(var(--teal)/0.9)] text-[hsl(var(--accent-foreground))]"
                >
                View Comparison
                </Button>
            )}
        ...
        {/* Product selection: tabs (single) vs segmented chips (compare) */}
        {!compareMode ? (
            <Tabs value={selectedProducts[0].toString()} onValueChange={(value) => handleProductSelect(parseInt(value))}>
            <TabsList className="grid w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 h-auto p-1 bg-[hsl(var(--cream))]">
                {products.map((product, index) => (
                <TabsTrigger
                    key={index}
                    value={index.toString()}
                    className="data-[state=active]:bg-forest data-[state=active]:text-cream text-sm py-3 px-4 rounded-md text-navy"
                >
                    {product.name}
                </TabsTrigger>
                ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                const disabled = !isSelected && selectedProducts.length >= 2;
                return (
                    <button
                    key={index}
                    ...
                    disabled={disabled}
                    className={`h-auto p-4 rounded-md text-left border transition-all ${
                        isSelected
                        ? 'bg-forest text-cream border-forest'
                        : 'bg-[hsl(var(--card))] text-navy border-[hsl(var(--border))] hover:bg-[hsl(var(--cream))]'
                    }`}
                    >
                    <div className="flex items-center gap-3">
                        <span className={`w-4 h-4 rounded-full border-2 ${isSelected ? 'bg-cream border-cream' : 'border-[hsl(var(--border))]'}`} />
            ...
            {/* Selected summary */}
            <div className="rounded-lg p-4 bg-[hsl(var(--cream))] border border-[hsl(var(--border))]" aria-live="polite">
                <h4 className="font-medium text-navy mb-2">Selected for comparison ({selectedProducts.length}/2):</h4>
                ...
                {selectedProducts.map((index) => (
                    ...
                    <button
                        onClick={() => handleProductSelect(index)}
                        className="ml-1 rounded-full p-0.5 hover:bg-[hsl(var(--cream)/0.2)]"
        ...
        {/* Mobile comparison sheet with dvh/svh + safe area */}
        <Sheet open={compareSheetOpen} onOpenChange={setCompareSheetOpen}>
            <SheetContent
            side="bottom"
            className="supports-[height:100svh]:h-[90svh] h-[90dvh] max-h-[min(92svh,800px)] pb-[max(env(safe-area-inset-bottom),1rem)] overflow-y-auto bg-[hsl(var(--card))]"
            >
            <SheetHeader>
                <SheetTitle className="text-navy">Product Comparison</SheetTitle>
                <SheetDescription className="text-[hsl(var(--steel))]">
                {canCompare ? `Compare ${primaryProduct.name} and ${secondaryProduct?.name}` : 'Select two products to compare'}
                </SheetDescription>
            </SheetHeader>
            {canCompare && (
                <div className="mt-6">
                <Tabs defaultValue="0" className="w-full">
                    <TabsList className="grid w-full grid-cols-2 bg-[hsl(var(--cream))]">
                    <TabsTrigger value="0" className="data-[state=active]:bg-forest data-[state=active]:text-cream text-navy">{primaryProduct.name}</TabsTrigger>
                    <TabsTrigger value="1" className="data-[state=active]:bg-forest data-[state=active]:text-cream text-navy">{secondaryProduct!.name}</TabsTrigger>
                    </TabsList>
        ...
        <ul className="space-y-1">
            ...
            <li key={s.id}>
                ...
                className={`block text-sm px-3 py-2 rounded-md transition-colors ${
                    active === s.id ? 'bg-[hsl(var(--forest)/0.10)] text-forest' : 'text-navy hover:bg-[hsl(var(--cream))]'
                }`}
                >
                {s.label}
                </a>
...
/* ---------------- Solution Detail (complete redesign) ---------------- */
...
if (!solution) {
    return (
        <Layout>
            ...
            <Link href="/solutions" className="text-teal hover:opacity-90 inline-flex items-center font-medium">
...
const toc = useMemo(
    () => [
        ...
        ...(isEdgeAISolution(solution) ? [{ id: 'products', label: 'Products' }] : []),
    ],
    [solution]
);
...
return (
    <Layout
        ...
        >
        {/* HERO — split masthead */}
        <section className="relative overflow-hidden bg-forest text-cream pt-20 sm:pt-24 lg:pt-28 pb-10 sm:pb-14">
            <div className="px-4 sm:px-6 md:px-8">
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)] gap-10 items-center">
                ...
                <motion.h1
                    ...
                    className="font-condensed font-bold leading-tight [text-wrap:balance] text-[clamp(1.8rem,6vw,3rem)]"
                >
                    {solution.title}
                </motion.h1>
                <motion.p
                    ...
                    className="mt-4 text-[clamp(1rem,2.6vw,1.25rem)] [text-wrap:pretty]"
                >
                    {solution.description}
                </motion.p>
                <div className="mt-5 flex flex-wrap gap-2">
                    {solution.certifications.map((c, i) => (
                    <span key={i} className="px-2.5 py-1 rounded-md bg-[hsl(var(--cream)/0.12)] text-cream text-xs font-medium border border-[hsl(var(--cream)/0.24)]">
                        {c}
                    </span>
                    ))}
                </div>
                ...
                <Card className="order-first lg:order-none bg-[hsl(var(--card))] text-navy border-0 shadow-lg">
                <CardHeader>
                    <div className="h-16 w-16 rounded-full bg-[hsl(var(--forest)/0.10)] flex items-center justify-center mb-4">
                    ...
                    <CardDescription className="text-[hsl(var(--steel))]">{solution.fullDescription.substring(0, 160)}...</CardDescription>
                </CardHeader>
                <CardContent className="pt-0">
                    <div className="grid grid-cols-2 gap-4">
                    <div>
                        <div className="text-xs uppercase tracking-wide text-[hsl(var(--steel))]">Category</div>
                        <div className="text-navy font-medium">Defense Technology</div>
                    </div>
                    <div>
                        <div className="text-xs uppercase tracking-wide text-[hsl(var(--steel))]">Certifications</div>
                        <div className="text-navy font-medium">{solution.certifications.length}</div>
                    </div>
                    </div>
                </CardContent>
                <CardFooter>
                    <Button asChild className="bg-teal hover:bg-[hsl(var(--teal)/0.9)] text-[hsl(var(--accent-foreground))] w-full">
        ...
        {/* BODY */}
        <section className="bg-[hsl(var(--card))] py-10 sm:py-14 lg:py-20">
            <div className="px-4 sm:px-6 md:px-8">
            <div className="max-w-6xl mx-auto min-w-0 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(260px,300px)] gap-10">
                {/* Main */}
                <div className="min-w-0">
                {/* Overview */}
                <div id="overview" className="scroll-mt-[88px] lg:scroll-mt-[112px] mb-8 sm:mb-10">
                    ...
                    className="font-condensed font-bold text-navy border-l-4 border-forest pl-4 mb-3 sm:mb-4 leading-tight [text-wrap:balance] text-[clamp(1.4rem,5vw,2.2rem)]"
                    >
                    Mission Overview
                    </motion.h2>
                    <motion.p
                    initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }} variants={fade}
                    className="text-[clamp(1rem,2.5vw,1.125rem)] max-w-3xl text-[hsl(var(--charcoal)/0.85)] [text-wrap:pretty]"
                    >
                    {solution.fullDescription}
                    </motion.p>

                    <div className="mt-5 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--cream))] p-4">
                    <div className="text-xs uppercase tracking-wide text-navy mb-1">{solution.mainQuestion}</div>
                    <div className="text-navy">{solution.mainAnswer}</div>
                    </div>
                </div>
                ...
                <Section id="features" title="Key Features">
                    <ul className="space-y-3 mb-8 text-[hsl(var(--charcoal)/0.9)]">
                ...
                <Section id="applications" title="Applications">
                    <ul className="space-y-3 mb-6 text-[hsl(var(--charcoal)/0.9)]">
                ...
                {/* CTA */}
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }} variants={fade} className="mt-12">
                    <Card className="bg-forest text-cream border-0 shadow-lg">
                    <CardContent className="p-6 sm:p-8">
                        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                        <div className="min-w-0">
                            <CardTitle className="text-xl lg:text-2xl font-bold mb-2 text-cream">Ready to implement this solution?</CardTitle>
                            <CardDescription className="text-sm md:text-base mb-0 text-[hsl(var(--cream)/0.9)]">
                            Our specialists can help you deploy a customized version for your specific needs.
                            </CardDescription>
                        </div>
                        <Button asChild size="lg" className="bg-teal hover:bg-[hsl(var(--teal)/0.9)] text-[hsl(var(--accent-foreground))] shrink-0">
                ...
                <Card className="mt-6 sticky top-28 bg-[hsl(var(--card))] border-[hsl(var(--border))]">
                    <CardHeader>
                    <div className="h-16 w-16 rounded-full bg-[hsl(var(--forest)/0.10)] flex items-center justify-center mb-4">
                        <svg className="h-8 w-8 text-forest" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                        <rect x="3" y="4" width="18" height="14" rx="2" />
                    ...
                    <CardDescription className="text-[hsl(var(--steel))]">{solution.description}</CardDescription>
                    </CardHeader>
                    <CardContent>
                    <div className="flex flex-wrap gap-2">
                        {solution.certifications.map((c, i) => (
...
/* ---------------- Solution List (complete redesign) ---------------- */
...
const categories = Array.from(new Set(Object.values(base).map((b) => categoryMap[b.id] || 'Other')));
...
const filtered = useMemo(() => {
    let items = base;
    if (activeCats.size) items = items.filter((p) => activeCats.has(categoryMap[p.id] || 'Other'));
    if (!q) return items;
...
const getIcon = (id: string) => (
    <svg className="w-10 h-10 sm:w-12 sm:h-12 text-forest" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <rect x="3" y="4" width="18" height="14" rx="2" />
    </svg>
);
...
return (
    <Layout
        ...
        >
        {/* HERO */}
        <section className="relative overflow-hidden bg-forest text-cream py-20 sm:py-24 lg:py-32 px-4 sm:px-6">
            <div className="max-w-6xl mx-auto text-center relative">
            <motion.h1
                className="text-[clamp(1.75rem,6vw,3rem)] font-condensed font-bold mb-5 tracking-tight [text-wrap:balance]"
                initial={{ opacity: 0, y: 18 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.65 }}
            >
            ...
            <motion.p
                className="text-[clamp(1rem,2.4vw,1.25rem)] text-[hsl(var(--cream)/0.85)] max-w-3xl mx-auto mb-8 sm:mb-10 [text-wrap:pretty]"
                initial={{ opacity: 0, y: 18 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.65, delay: 0.15 }}
            >
            ...
            >
                {['MIL-STD-810', 'AES-256', 'FIPS 140-2', 'MIL-STD-461'].map((c, i) => (
                <span key={i} className="px-2.5 py-1 rounded-md bg-[hsl(var(--cream)/0.12)] text-cream text-xs font-medium border border-[hsl(var(--cream)/0.24)]">
                    {c}
                </span>
                ))}
        ...
        {/* Filters + Results */}
        <section className="bg-[hsl(var(--card))] py-10 sm:py-16 lg:py-20 px-4 sm:px-6">
            <div className="max-w-7xl mx-auto">
            {/* Mobile Filters Sheet */}
            <div className="lg:hidden mb-6">
                <div className="flex gap-3 mb-4">
                <div className="relative flex-1">
                    <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-[hsl(var(--steel))]" />
                    <input
                    type="search"
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    placeholder="Search solutions..."
                    className="w-full pl-9 pr-3 py-2.5 rounded-md border border-[hsl(var(--input))] focus:outline-none focus:ring-2 focus:ring-forest focus:border-transparent text-sm text-navy bg-[hsl(var(--card))]"
                    aria-label="Search solutions"
                    />
                </div>
                <Sheet>
                    <SheetTrigger asChild>
                    <Button variant="outline" size="sm" className="shrink-0 text-navy border-[hsl(var(--border))]">
                        <FaFilter className="w-4 h-4 mr-2" />
                        Filters
                        {activeCats.size > 0 && (
                        <span className="ml-2 px-2 py-0.5 rounded bg-forest text-cream text-xs">{activeCats.size}</span>
                        )}
                    </Button>
                    </SheetTrigger>
                    <SheetContent side="right" className="w-80 bg-[hsl(var(--card))] text-navy">
                    <SheetHeader>
                        <SheetTitle className="text-navy">Filter Solutions</SheetTitle>
                        <SheetDescription className="text-[hsl(var(--steel))]">Narrow down solutions by category</SheetDescription>
                    </SheetHeader>
                    <div className="mt-6 space-y-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            return <Pill key={c} active={active} onClick={() => toggleCategory(c)}>{c}</Pill>;
                            })}
                        </div>
                        {activeCats.size > 0 && (
                            <Button
                            ...
                            className="w-full mt-4 text-[hsl(var(--alert))] hover:bg-[hsl(var(--alert)/0.08)]"
                            >
                            Clear all filters
                            </Button>
                        )}
                        </div>
                        <Card className="p-4 bg-[hsl(var(--cream))] border-[hsl(var(--border))]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-10">
                ...
                <Card className="sticky top-24 bg-[hsl(var(--card))] border-[hsl(var(--border))]">
                    <CardHeader>
                    <CardTitle className="text-lg text-navy">Filter Solutions</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-6">
                    <div>
                    ...
                    <div>
                        ...
                        <Button
                            ...
                            size="sm"
                            onClick={clearFilters}
                            className="w-full mt-3 text-[hsl(var(--alert))] hover:bg-[hsl(var(--alert)/0.08)]"
                        >
                            Clear all filters
                        </Button>
                        )}
                    ...
                    <Separator className="bg-[hsl(var(--border))]" />
                    <div className="rounded-md p-3 text-sm bg-[hsl(var(--cream))]">
                        <p className="font-medium text-navy mb-2">How to use</p>
                        <p className="text-[hsl(var(--steel))] text-xs leading-relaxed">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                    <h2 className="text-[clamp(1.4rem,4vw,2.2rem)] font-condensed font-bold text-navy leading-tight mb-2 [text-wrap:balance]">
                        Our Defense Technology Solutions
                    </h2>
                    <p className="text-sm text-[hsl(var(--steel))]">
                        {resultsLabel}
                        {activeCats.size ? ' • Filtered' : ''}
                        {q ? ` • Matching '${query}'` : ''}
                    </p>
                    </div>
                    <div className="flex items-center gap-3 self-start sm:self-auto">
                    <div className="inline-flex rounded-md border border-[hsl(var(--border))] overflow-hidden" role="group" aria-label="Change results view">
                        <button
                        onClick={() => setView('grid')}
                        className={`px-3 py-2 text-xs font-medium tracking-wide ${
                            view === 'grid' ? 'bg-forest text-cream' : 'bg-[hsl(var(--card))] text-navy hover:bg-[hsl(var(--cream))]'
                        }`}
                        ...
                        className={`px-3 py-2 text-xs font-medium tracking-wide border-l border-[hsl(var(--border))] ${
                            view === 'list' ? 'bg-forest text-cream' : 'bg-[hsl(var(--card))] text-navy hover:bg-[hsl(var(--cream))]'
                        }`}
                ...
                {/* Empty State */}
                {results.length === 0 && (
                    <div className="text-center bg-[hsl(var(--cream))] border border-[hsl(var(--border))] rounded-lg p-10">
                    <p className="text-navy font-medium mb-2">No matching solutions</p>
                    <p className="text-sm text-[hsl(var(--steel))] mb-6 max-w-md mx-auto">Try different keywords or remove some category filters to broaden the results.</p>
                    {(activeCats.size > 0 || q) && (
                        <button
                        onClick={() => {
                            clearFilters();
                        ...
                        className="bg-forest text-cream px-4 py-2 rounded-md text-sm font-medium hover:opacity-90"
                        >
                        Reset Search
                        </button>
                    )}
                ...
                {results.length > 0 && view === 'grid' && (
                    ...
                    {results.map((p, i) => (
                        ...
                        >
                        <Card className="group h-full bg-[hsl(var(--card))] border-[hsl(var(--border))] hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                            <CardHeader className="pb-4">
                            <div className="aspect-[16/9] mb-4 rounded-lg bg-[hsl(var(--cream))] grid place-items-center relative overflow-hidden">
                                <div className="w-16 h-16 rounded-full bg-[hsl(var(--forest)/0.10)] flex items-center justify-center">
                                {getIcon(p.id)}
                                </div>
                            ...
                            <CardTitle className="text-lg font-condensed font-bold text-navy leading-tight">
                                <Link href={`/solutions/${p.id}`} className="hover:underline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-forest rounded-sm">
                                {p.title}
                                </Link>
                            </CardTitle>
                            <CardDescription className="text-sm text-[hsl(var(--steel))] line-clamp-2">{p.description}</CardDescription>
                            </CardHeader>

                            <CardContent className="flex-grow">
                            <ul className="space-y-2 text-sm text-[hsl(var(--charcoal)/0.9)]">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                ...
                                <span key={j} className="px-2 py-1 rounded bg-[hsl(var(--forest)/0.10)] text-forest text-xs font-medium">{c}</span>
                                ))}
                                {p.certifications.length > 2 && (
                                <span className="px-2 py-1 rounded border border-[hsl(var(--border))] text-navy text-xs">+{p.certifications.length - 2}</span>
                                )}
                            </div>
                            <Button asChild size="sm" className="bg-forest hover:opacity-90 text-cream">
                                <Link href={`/solutions/${p.id}`}>Explore</Link>
                            </Button>
                            </CardFooter>
                        </Card>
                        </motion.div>
                    ...
                    {results.map((p, i) => (
                        ...
                        <Card className="bg-[hsl(var(--card))] border-[hsl(var(--border))] hover:shadow-md transition-shadow">
                            ...
                            <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                                {/* Left side - Visual and badges */}
                                <div className="md:w-48 shrink-0">
                                <div className="aspect-square rounded-lg bg-[hsl(var(--cream))] grid place-items-center mb-4">
                                    <div className="w-16 h-16 rounded-full bg-[hsl(var(--forest)/0.10)] flex items-center justify-center">{getIcon(p.id)}</div>
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                    ...
                                    <span className="px-2 py-1 rounded border border-[hsl(var(--border))] text-navy text-xs">+{p.certifications.length - 3}</span>
                                    )}
                                </div>
                                </div>
                                ...
                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                                    ...
                                    <CardDescription className="text-sm md:text-base text-[hsl(var(--charcoal)/0.85)] max-w-3xl">{p.fullDescription}</CardDescription>
                                    </div>
                                    <Button asChild className="bg-forest hover:opacity-90 text-cream shrink-0">
                                    <Link href={`/solutions/${p.id}`}>View Details</Link>
                                    </Button>
                                </div>
                                ...
                                <div>
                                    ...
                                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm text-[hsl(var(--charcoal)/0.9)]">
                                    {p.features.map((f, idx) => (
                                        <li key={idx} className="flex items-start gap-2">
                                        <FaCheck className="text-forest mt-0.5 shrink-0 text-xs" />
                                        <span>{f}</span>
        ...
        {/* CTA */}
        <section className="bg-[hsl(var(--cream))] py-12 sm:py-16 lg:py-20 px-4 sm:px-6">
            <div className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.6 }}>
                <Card className="bg-[hsl(var(--card))] border-0 shadow-lg">
                <CardContent className="p-8 lg:p-12">
                    <div className="flex flex-col lg:flex-row lg:items-center gap-8 lg:gap-14">
                    <div className="lg:w-2/3 min-w-0">
                        <CardTitle className="text-2xl lg:text-3xl font-bold mb-4 text-navy leading-tight [text-wrap:balance]">Need a Custom Defense Solution?</CardTitle>
                        <CardDescription className="text-base text-[hsl(var(--charcoal)/0.85)] mb-0 [text-wrap:pretty]">
                    ...
                    <div className="lg:w-1/3 flex justify-center lg:justify-end">
                        <Button asChild size="lg" className="bg-navy hover:opacity-90 text-cream w-full lg:w-auto">
